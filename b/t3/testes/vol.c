...
void Busca(Registro Reg, Apontador Ap)
{
    ...
    if (Reg.chave < Ap->r[i - 1].chave)  Busca(Reg, Ap->p[i-1]);
    else
    Busca(Reg, Ap->p[i]);
}
...
void buscainFile(Registro Reg, Apontador pagina)
{
    ...
    fclose(arq);
    for(i = 0; i < 2*ORDEM; i++){
        if (Reg.chave == reg[i].chave)      printf("%s%d\n",reg[i].nome,reg[i].idade);
    }
}
...
void Ins(Registro Reg, Apontador Ap, int *Cresceu, Registro *RegRetorno, Apontador *ApRetorno)
{
    ...
    if (Reg.chave < Ap->r[i - 1].chave)  Ins(Reg, Ap->p[i-1], Cresceu, RegRetorno, ApRetorno);
    else
    ...
    for (j = ORDEM + 2; j <= 2*ORDEM; j++)  InsereNaPagina(ApTemp, Ap->r[j - 1], Ap->p[j]);
...
int Imprime2(Apontador p, int Nivel, int aux, int n)
{
    ...
    if(Nivel == aux){
        ...
        for (i = 0; i < p->n; i++){
            if (p->p[i] !=NULL)          printf("Apontador: %d ", p->p[i]->pageNum);
            else
        ...
        aux++;
        for (i = 0; i <= p->n; i++)      Imprime2(p->p[i], Nivel, aux, n);
    }
    return 0;
}
...
int Imprime(Apontador p, int N, int aux, int n)
{
    ...
    if(p->pageNum != aux){
        ...
        aux++;
        for (i = 0; i <= p->n; i++)      Imprime(p->p[i], N, aux, n);
    }
    return 0;
}
...
void InsPosFile(Registro Reg, Apontador Ap, int *Cresceu, Registro *RegRetorno, Apontador *ApRetorno)
{
    ...
    if (Reg.chave < Ap->r[i - 1].chave)  InsPosFile(Reg, Ap->p[i-1], Cresceu, RegRetorno, ApRetorno);
    else
...
void Reconstitui(Apontador ApPag, Apontador ApPai, int PosPai, int *Diminuiu)
{
    ...
    if (PosPai < ApPai->n) {  /* Aux = Pagina a direita de ApPag */
        ...
        if (DispAux > 0) {  /* Existe folga: transfere de Aux para ApPag */
            for (j = 1; j < DispAux; j++)          InsereNaPagina(ApPag, Aux->r[j - 1], Aux->p[j]);
        ...
        { /* Fusao: intercala Aux em ApPag e libera Aux */
            for (j = 1; j <= ORDEM; j++)          InsereNaPagina(ApPag, Aux->r[j - 1], Aux->p[j]);
            free(Aux);
    ...
    { /* Aux = Pagina a esquerda de ApPag */
        ...
        {  /* Fusao: intercala ApPag em Aux e libera ApPag */
            for (j = 1; j <= ORDEM; j++)          InsereNaPagina(Aux, ApPag->r[j - 1], ApPag->p[j]);
            free(ApPag);
            ApPai->n--;
            if (ApPai->n >= ORDEM)          *Diminuiu = 0;
        }
...
void Ret(int Ch, Apontador *Ap, int *Diminuiu)
{
    ...
    if (Ch == WITH->r[Ind - 1].chave)  {
        ...
        if (*Diminuiu)      Reconstitui(WITH->p[Ind - 1], *Ap, Ind - 1, Diminuiu);
        return;
    }
...
void recuperarReg(Apontador *arv, node *LISTA)
{
    ...
    while (j*(2*ORDEM*sizeof(Registro)) < tam){
        ...
        for(i = 0; i < 2*ORDEM; i++){
            if (Reg[i].chave > 0)          execut(Reg[i], LISTA);
        }
        j++;
    }
    fclose(arq);
...
void saveAux(Apontador p, int Nivel)
{
    ...
    if (p == NULL)  return;
    for (i = 0; i < p->n; i++)  salvar(p, p->r);
    for (j = 0; j <= p->n; j++)  saveAux(p->p[j], Nivel + 1);
}
